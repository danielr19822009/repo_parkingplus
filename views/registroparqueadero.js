...
formElementparking.addEventListener("submit", async (event) => {
  ...
  const data = { nombreparqueadero, direccion, capacidad };

  try {
    const response = await axios.post("http://localhost:3002/parqueadero", data);

    Swal.fire({
      title: "Parqueadero creado",
      html: `<strong>${nombreparqueadero}</strong> registrado correctamente`,
      icon: "success",
    }).then(() => {
